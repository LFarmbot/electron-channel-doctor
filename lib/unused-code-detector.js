...
class UnusedCodeDetector {
     ...
     */
    scanProjectFiles() {
        const jsFiles = this.fs.findFiles(this.options.jsPattern, this.options.projectRoot, this.options.ignore);
        const cssFiles = this.fs.findFiles(this.options.cssPattern, this.options.projectRoot, this.options.ignore);
        const htmlFiles = this.fs.findFiles(this.options.htmlPattern, this.options.projectRoot, this.options.ignore);
    ...
    async detectUnusedFunctions(jsFiles) {
        ...
        const fileContents = await Promise.all(
            jsFiles.map(filePath => this.fs.readFile(filePath))
        );
    ...
    async detectUnusedCssClasses(cssFiles, htmlFiles, jsFiles) {
        ...
        const [cssContents, htmlContents, jsContents] = await Promise.all([
            Promise.all(cssFiles.map(filePath => this.fs.readFile(filePath))),
            Promise.all(htmlFiles.map(filePath => this.fs.readFile(filePath))),
            Promise.all(jsFiles.map(filePath => this.fs.readFile(filePath)))
        ]);
    ...
    async detectDeadCodePaths(jsFiles) {
        ...
        await Promise.all(jsFiles.map(async (filePath) => {
            ...
            // Code after return statements}]+)/g);
