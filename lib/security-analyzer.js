...
class SecurityAnalyzer {
    ...
    async analyzeMainProcess() {
        ...
        for (const file of files) {
            let ast, code;
            try {
                code = await this.fs.readFile(file);
                ast = parse(code, {
                    sourceType: 'module',
                ...
                if (this.options.verbose) {
                    console.warn(`Could not parse file for security analysis: ${file} - ${error.message}`);
                }
                continue;
            }
    ...
    async analyzeIPCPatterns() {
        ...
        const rendererFiles = this.findFiles(this.options.rendererProcess);
     ...
     */
    findFiles(patterns) {
        return this.fs.findFiles(patterns, this.options.projectRoot, this.options.ignore);
    }
            ...
            NewExpression(nodePath) {
                if (nodePath.get('callee').isIdentifier({ name: 'BrowserWindow' })) {
                    ...
                    if (args.length > 0 && args[0].isObjectExpression()) {
                        const webPreferences = args[0].get('properties').find(
                            p => p.isObjectProperty() && p.get('key').isIdentifier({ name: 'webPreferences' })
                        );

                        if (webPreferences) {
                            const webPrefValue = webPreferences.get('value');
                            if (webPrefValue.isObjectExpression()) {
                                webPrefValue.get('properties').forEach(prop => {
                                    if (!prop.isObjectProperty()) return;
                                    const key = prop.get('key');
                                    const value = prop.get('value');

                                    if (key.isIdentifier({ name: 'nodeIntegration' }) && value.isBooleanLiteral({ value: true })) {
                                        issues.push({
                                            ...
                                            recommendation: 'Set nodeIntegration: false for security'
                                        });
                                    }
                                    if (key.isIdentifier({ name: 'contextIsolation' }) && value.isBooleanLiteral({ value: false })) {
                                        issues.push({
                                            type: 'disabled-context-isolation',
                                            severity: 'critical',
                                            file: filePath,
                                            line: prop.node.loc.start.line,
                                            message: 'contextIsolation disabled - essential for security',
                                            recommendation: 'Set contextIsolation: true in webPreferences'
                                        });
                                    }
                                    if (key.isIdentifier({ name: 'webSecurity' }) && value.isBooleanLiteral({ value: false })) {
                                        issues.push({
                                            type: 'disabled-web-security',
                                            severity: 'high',
                                            file: filePath,
                                            line: prop.node.loc.start.line,
                                            message: 'webSecurity disabled - allows cross-site scripting',
                                            recommendation: 'Only disable webSecurity if absolutely necessary'
                                        });
                                    }
                                });
                            }
