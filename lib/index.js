...
const { default: traverseModule } = require('@babel/traverse');
...
class ChannelDoctor {
    ...
    async getCurrentWhitelist() {
        ...
        let preloadContent;
        try {
            preloadContent = await fs.readFile(preloadPath, 'utf8');
        } catch (error) {
    ...
    async scanForInvokeCalls() {
        ...
        let files = glob.sync(jsPattern);
        files = files.filter(file => !defaultIgnore.some(pattern => file.includes(pattern)));
